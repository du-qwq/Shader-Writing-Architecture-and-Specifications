Shader "ToonRenderFramework/Character/Lit/ToonCharacterLit"
{
    Properties
    {
        ...
        _ShadowColor("Shadow Color (Local Backup)", Color) = (0.72, 0.78, 0.90, 1)
        _ToonThreshold("Toon Threshold (Local Backup)", Range(0, 1)) = 0.5
        _ToonSmoothness("Toon Smoothness (Local Backup)", Range(0.001, 0.3)) = 0.05

        [Toggle(_TOON_SPECULAR_ON)] _EnableSpecular("Enable Specular", Float) = 1
        _SpecColor("Specular Color", Color) = (1,1,1,1)
        _SpecThreshold("Specular Threshold", Range(0, 1)) = 0.78
        _SpecSmoothness("Specular Smoothness", Range(0.001, 0.2)) = 0.03
        _SpecIntensity("Specular Intensity", Range(0, 2)) = 0.6

        [Toggle(_TOON_RIM_ON)] _EnableRim("Enable Rim", Float) = 1
        _RimColor("Rim Color", Color) = (1,1,1,1)
        _RimPower("Rim Power", Range(0.1, 8)) = 3.0
        _RimIntensity("Rim Intensity", Range(0, 2)) = 0.5

        _OutlineColor("Outline Color", Color) = (0,0,0,1)
        _OutlineWidth("Outline Width", Range(0, 0.05)) = 0.004
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _TOON_SPECULAR_ON
            #pragma shader_feature_local _TOON_RIM_ON
            ...
            // 全局 Toon 参数：由 ToonRenderSettings / LightingPresetController 控制
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                half3 rim = 0;
                #if defined(_TOON_RIM_ON)
                {
                    half rimTerm = 1.0h - saturate(dot(normalWS, viewDirWS));
                    rimTerm = pow(rimTerm, _RimPower);
                    rim = rimTerm * _RimColor.rgb * _RimIntensity;
                }
                #endif

                half3 finalColor = ambient + toonDiffuse + specular + rim;
